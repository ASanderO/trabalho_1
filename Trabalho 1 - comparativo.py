...
# Função para gerar vetores de inteiros aleatórios no intervalo de 0 a 99999
...
def avaliar_algoritmo_ordenacao(algoritmo_ordenacao, vetores_execucoes, num_execucoes=10):
    logger.info(f'Avaliando algoritmo de ordenação: {algoritmo_ordenacao.__name__}')
    tamanhos = vetores_execucoes[0].keys()
    ...
    for execucao in range(num_execucoes):
        ...
        for tamanho in tamanhos:
            arr = vetores_execucoes[execucao][tamanho].copy()
            logger.info(f'Vetor gerado de tamanho {tamanho}: {arr[:10]}...')
            t1 = time.perf_counter()
            sorted_arr, n_trocas, n_iteracoes = algoritmo_ordenacao(arr)
...
def avaliar_algoritmo_busca(algoritmo_busca, vetores_execucoes, num_execucoes=10):
    ...
    for execucao in range(num_execucoes):
        logger.info(f'Execução {execucao + 1} de {num_execucoes}')
        for tamanho in tamanhos:
            vetor_desordenado = vetores_execucoes[execucao][tamanho].copy()
...
if __name__ == "__main__":
    ...
    # Gerar vetores aleatórios para cada execução e tamanho
    vetores_execucoes = [{tamanho: gerar_vetor_aleatorio(tamanho) for tamanho in tamanhos} for _ in range(num_execucoes)]

    for algoritmo in algoritmos_ordenacao:
        avaliar_algoritmo_ordenacao(algoritmo, vetores_execucoes, num_execucoes)

    for algoritmo_busca in algoritmos_busca:
        avaliar_algoritmo_busca(algoritmo_busca, vetores_execucoes, num_execucoes)
